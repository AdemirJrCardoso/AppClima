...
export class CoordenadaProvider {
  ...
  async getClimate(lat, lon) {
    try {
      const response: any = await this.http
        .get(
          `https://api.openweathermap.org/data/2.5/weather?lat=${lat}&lon=${lon}&appid=bbd4f0cb4a5c7d4156f89123be8cbe92&lang=pt&units=metric`
        )
        .toPromise();
      return {
  ...
  async getClimateByName(city) {
    try {
      const response: any = await this.http
        .get(
          `https://api.openweathermap.org/geo/1.0/direct?q=${city}&limit=5&appid=bbd4f0cb4a5c7d4156f89123be8cbe92`
        )
        .toPromise();
      if (!response.length) {
